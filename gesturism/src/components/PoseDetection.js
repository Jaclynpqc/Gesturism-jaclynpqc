...
/** Import pose estimation with PoseNet */
import * as posenet from "@tensorflow-models/posenet";

// The PoseDection class is designed to handle pose estimation using the PoseNet model from TensorFlow.js.
// It manages video input, model initialization, and pose detection while providing a GUI for configuration. 
export default class PoseDetection {
    constructor(video, mobile){
        this.video = video; 
        this.guiState;
        this.isMobile = mobile; // A bool indicating whether the device is mobile
        this.algorithm = "single"; // "single" or "multi" pose
        ...
        this.smoothPrevPose; // Array to store previously estimated poses for smoothing
        this.smoothPrevPoses = [];
    }

    // Load the PoseNet model with specific configurations
    async initPoseDetection(){
        this.net = await posenet.load({
            // MobileNetV1 is used for real-time performance, especially on mobile devices
            architecture: 'MobileNetV1',
            // Output Stride: Affects the resolution of the output; a lower value increases accuracy but decreases speed
            outputStride:16,
    ...
    // Initializes the GUI to allow user control over various parameters
    // May adapt it to my web app later for further styling
    initGui(gui){
        this.guiState = {
            smoothing: this.smoothFactor,
            input:{
                mobileNetArchitecture: this.isMobile? "0.5": "0.75",
                outputStride: 16, 
        ...
        // Inspired by PoseNet sketch by Maya Man at Google Creative Lab,
        // This system supports single or multi people mode. 
        // The single mode only works if only one person in frame or results will be inaccurat
        // Multi-pose works for more than 1 person
        ...
        // 1.01 is the largest, but will be the slowest
        // 0.50 is the fastest, but least accurate
        ...
        // the higher the accuracy but slower the speed, the higher the value
        // the faster the speed but lower accuracy. 
        ...
        // Pose confidence: the overall confidence in the estimationof a person's pose
        // (i.e. a person detected in a frame)
    ...
    async getPoses() {
        ...
        const outputStride = +this.guiState.input.ouputStride;
        const flipHorizontal = this.flipHorizontal; 
    ...
    getKeypoints(pose, parts){
        ...
        if (parts) {
            ...
            pose.keypoints.forEach((keypoint)=> {
                keypoints[keypoints.part] = this.getKeypoints(pose, keypoint.part);
            });
        }
        return keypoints;
    }
